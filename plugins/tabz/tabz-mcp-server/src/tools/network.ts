...
export function registerNetworkTools(server: McpServer): void {
  ...
  server.tool(
    ...
    `Enable network request monitoring. Call BEFORE browsing, then use tabz_get_network_requests to inspect.
...
Make sure the Chrome extension is installed and connected.`
  ...
  server.tool(
    ...
    `List captured network requests. Call tabz_enable_network_capture first.

Args: urlPattern (optional), method (optional), statusMin/statusMax (optional), resourceType/limit/offset/tabId (optional)`,
...
3. Call this tool again to see captured requests`
  ...
  server.tool(
    ...
    `Clear all captured network requests. Capture remains active after clearing.`,
