...
export function registerInteractionTools(server) {
    // Click tool
    server.tool("tabz_click", `Click an element on the page.
...
or tabz_execute_script to verify the result.`, ClickSchema.shape, async (params) => {
        try {
...
- Element may be hidden or not clickable`;
    ...
    // Fill tool
    server.tool("tabz_fill", `Fill an input field with text.
...
- Verify the value: tabz_execute_script with code="document.querySelector('${"{selector}"}').value"`, FillSchema.shape, async (params) => {
        try {
