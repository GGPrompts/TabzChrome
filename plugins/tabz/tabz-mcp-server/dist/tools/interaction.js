...
export function registerInteractionTools(server) {
    // Click tool
    server.tool("tabz_click", `Click an element on the page. Waits up to 5 seconds for element to appear.

Args: selector (required CSS selector), tabId (optional)`, ClickSchema.shape, async (params) => {
        try {
...
- Element may be hidden or not clickable`;
    ...
    // Fill tool
    server.tool("tabz_fill", `Fill an input field with text. Clears existing value first. Works with inputs, textareas, contenteditable.

Args: selector (required CSS selector), value (required text), tabId (optional)`, FillSchema.shape, async (params) => {
        try {
