 ...
 */
async function listPlugins(marketplace, enabled) {
    try {
        const response = await axios.get(`${BACKEND_URL}/api/plugins`, { timeout: 10000 });
        ...
        if (enabled !== undefined) {
            for (const [mktName, plugins] of Object.entries(marketplaces)) {
                marketplaces[mktName] = plugins.filter((p) => p.enabled === enabled);
            }
 ...
 */
async function listSkills(plugin, search) {
    try {
        const response = await axios.get(`${BACKEND_URL}/api/plugins/skills`, { timeout: 10000 });
        ...
        if (plugin) {
            ...
            skills = skills.filter((s) => s.pluginName.toLowerCase() === pluginLower);
        }
 ...
 */
async function pluginsHealth() {
    try {
        const response = await axios.get(`${BACKEND_URL}/api/plugins/health`, { timeout: 30000 });
...
async function togglePlugin(pluginId, enabled) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/plugins/toggle`, { pluginId, enabled }, { timeout: 5000 });
        return response.data;
    }
    catch (error) {
...
Use tabz_get_skill to read the full SKILL.md content.`, ListSkillsSchema.shape, async (params) => {
    ...
    server.tool("tabz_get_skill", `Get full details and content of a specific skill.
...
Use tabz_list_skills first to find available skill IDs.`, GetSkillSchema.shape, async (params) => {
        try {
