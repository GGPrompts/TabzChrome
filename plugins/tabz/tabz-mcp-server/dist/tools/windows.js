...
async function listWindows() {
    try {
        const response = await axios.get(`${BACKEND_URL}/api/browser/windows`, { timeout: 10000 });
...
async function createWindow(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/windows`, options, { timeout: 15000 });
        return response.data;
    }
    catch (error) {
...
async function updateWindow(options) {
    try {
        const response = await axios.put(`${BACKEND_URL}/api/browser/windows/${options.windowId}`, options, { timeout: 10000 });
        return response.data;
    }
    catch (error) {
...
async function closeWindow(windowId) {
    try {
        const response = await axios.delete(`${BACKEND_URL}/api/browser/windows/${windowId}`, { timeout: 10000 });
        return response.data;
    }
    catch (error) {
...
async function getDisplays() {
    try {
        const response = await axios.get(`${BACKEND_URL}/api/browser/displays`, { timeout: 10000 });
        return response.data;
    }
    catch (error) {
...
async function tileWindows(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/windows/tile`, options, { timeout: 15000 });
        return response.data;
    }
    catch (error) {
...
async function popoutTerminal(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/popout-terminal`, options, { timeout: 15000 });
        return response.data;
    }
    catch (error) {
...
export function registerWindowTools(server) {
    // List windows tool
    server.tool("tabz_list_windows", `List all Chrome windows with dimensions, state, and tab counts.

Args: response_format (optional)`, ListWindowsSchema.shape, async (params) => {
        try {
            ...
            if (params.response_format === ResponseFormat.JSON) {
                ...
                if (result.windows.length === 0) {
                    ...
                    const lines = [`# Chrome Windows (${result.windows.length})`, ""];
    ...
    // Create window tool
    server.tool("tabz_create_window", `Create a new Chrome window. Use type="popup" for minimal UI. Use url="/sidepanel/sidepanel.html" for terminal popout.

Args: url (optional), type (optional), state/focused/width/height/left/top/incognito/tabId (optional)`, CreateWindowSchema.shape, async (params) => {
        try {
...
**Error:** ${result.error}`;
    ...
    // Update window tool
    server.tool("tabz_update_window", `Resize, move, focus, or change state of a Chrome window. Get IDs from tabz_list_windows.

Args: windowId (required), state/focused/width/height/left/top/drawAttention (optional)`, UpdateWindowSchema.shape, async (params) => {
        try {
...
Use \`tabz_list_windows\` to get valid window IDs.`;
    ...
    // Close window tool
    server.tool("tabz_close_window", `Close a Chrome window and ALL its tabs. Get IDs from tabz_list_windows.

Args: windowId (required)`, CloseWindowSchema.shape, async (params) => {
        try {
...
Use \`tabz_list_windows\` to get valid window IDs.`;
    ...
    // Get displays tool
    server.tool("tabz_get_displays", `Get connected monitor/display info -- dimensions, positions, work areas. Use for multi-monitor window placement.

Args: response_format (optional)`, GetDisplaysSchema.shape, async (params) => {
        try {
            ...
            if (params.response_format === ResponseFormat.JSON) {
                ...
                if (result.displays.length === 0) {
                    ...
                    const lines = [`# Displays (${result.displays.length})`, ""];
                    for (const display of result.displays) {
                        ...
                        if (display.rotation !== 0)
                            lines.push(`- **Rotation:** ${display.rotation}°`);
                        lines.push("");
                    }
                    lines.push("---");
    ...
    // Tile windows tool
    server.tool("tabz_tile_windows", `Auto-arrange windows in horizontal, vertical, or grid layout on a display.

Args: windowIds (required array), layout (optional: horizontal/vertical/grid), displayId (optional), gap (optional)`, TileWindowsSchema.shape, async (params) => {
        try {
...
Use \`tabz_list_windows\` to get valid window IDs.`;
    ...
    server.tool("tabz_popout_terminal", `Pop terminal sidebar into a standalone popup window. Shares extension instance -- no conflicts.

Args: terminalId (optional), width/height/left/top (optional)`, PopoutTerminalSchema.shape, async (params) => {
        try {
