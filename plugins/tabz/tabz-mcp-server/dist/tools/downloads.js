...
export function registerDownloadTools(server) {
    // Download file tool
    server.tool("tabz_download_file", `Download a file from any URL using Chrome's download manager.
...
Error Handling:
  ...
  - "Download timed out": File too large or slow connection (may still complete)`, DownloadFileSchema.shape, async (params) => {
        try {
...
- Check Chrome's download settings aren't blocking this file type`;
    ...
    server.tool("tabz_get_downloads", `List recent downloads from Chrome's download manager.
...
Use tabz_cancel_download with the download ID to cancel in-progress downloads.`, GetDownloadsSchema.shape, async (params) => {
        try {
    ...
    server.tool("tabz_cancel_download", `Cancel an in-progress download.
...
Note: Cancelled downloads cannot be resumed. You'll need to start a new download.`, CancelDownloadSchema.shape, async (params) => {
        try {
...
The download may have already completed or been cancelled.`;
    ...
    server.tool("tabz_save_page", `Save the current browser tab as an MHTML file for offline analysis.
...
For security, browsers won't load MHTML files from web origins.`, SavePageSchema.shape, async (params) => {
        try {
