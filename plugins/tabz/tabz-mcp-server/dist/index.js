 ...
 */
async function getPresetTools(presetName) {
    try {
 ...
 */
function createFilteredServer(server, enabledTools) {
    ...
    serverAny.tool = function (...allArgs) {
        const name = allArgs[0];
    ...
    if (serverAny.registerTool) {
        ...
        serverAny.registerTool = function (...allArgs) {
            const name = allArgs[0];
            if (enabledTools.has(name)) {
...
async function main() {
    ...
    server.tool('tabz_docs', 'Get detailed documentation for any tabz MCP tool including parameters, constraints, examples, and error handling.', { tool_name: z.string().describe('Tool name to look up, e.g. "tabz_screenshot"') }, async ({ tool_name }) => {
