...
if [ -n "$LATEST_RELEASE" ] && version_gt "$LATEST_RELEASE" "$CURRENT_VERSION"; then
    ...
    if [[ "$DO_UPDATE" =~ ^[Yy]$ ]]; then
        ...
        if ! git diff --quiet || ! git diff --cached --quiet; then
            ...
            NON_LOCK_CHANGES=$(echo "$CHANGED_FILES" | grep -v '^package-lock.json$' | grep -v '^$' || true)

            if [ -z "$NON_LOCK_CHANGES" ]; then
                # Only package-lock.json changed - auto-reset it (npm install will regenerate)
                echo -e "${YELLOW}📦 Auto-resetting package-lock.json (will be regenerated by npm install)${NC}"
                git restore package-lock.json 2>/dev/null || git checkout -- package-lock.json 2>/dev/null || true
            else
                # Real changes detected - handle with stash
