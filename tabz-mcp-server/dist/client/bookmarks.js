...
export async function getBookmarkTree(options) {
    try {
        ...
        const response = await axios.get(`${BACKEND_URL}/api/browser/bookmarks/tree?${params.toString()}`, { timeout: 10000 });
...
export async function searchBookmarks(options) {
    try {
        ...
        const response = await axios.get(`${BACKEND_URL}/api/browser/bookmarks/search?${params.toString()}`, { timeout: 10000 });
        return response.data;
    }
    catch (error) {
...
export async function createBookmark(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/bookmarks/create`, {
            url: options.url,
            title: options.title,
...
export async function createBookmarkFolder(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/bookmarks/create-folder`, {
            title: options.title,
            parentId: options.parentId || '1', // Default to Bookmarks Bar
            index: options.index
        }, { timeout: 10000 });
...
export async function moveBookmark(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/bookmarks/move`, {
            id: options.id,
