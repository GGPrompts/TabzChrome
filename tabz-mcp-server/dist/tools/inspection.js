 ...
 */
async function getElementInfo(selector, options = {}) {
    try {
 ...
 */
function formatElementAsMarkdown(info, selector) {
    if (!info.success) {
 ...
 */
export function registerInspectionTools(server) {
    server.tool("tabz_get_element", `Inspect a DOM element -- returns HTML, computed styles, bounds, and attributes. Ideal for CSS debugging.

Args: selector (required), includeStyles (optional, default true), styleProperties (optional array), response_format (optional)`, GetElementSchema.shape, async (params) => {
        try {
