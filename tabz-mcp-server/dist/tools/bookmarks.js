...
export function registerBookmarkTools(server) {
    // Get bookmark tree
    server.tool("tabz_get_bookmark_tree", `Get the Chrome bookmarks hierarchy.
...
Use the returned IDs with tabz_save_bookmark, tabz_move_bookmark, etc.`, GetBookmarkTreeSchema.shape, async (params) => {
        try {
    ...
    // Search bookmarks
    server.tool("tabz_search_bookmarks", `Search Chrome bookmarks by title or URL.
...
Use the returned IDs with tabz_move_bookmark or tabz_delete_bookmark.`, SearchBookmarksSchema.shape, async (params) => {
        try {
    ...
    // Save bookmark
    server.tool("tabz_save_bookmark", `Save a URL as a Chrome bookmark.
...
The bookmark ID can be used with tabz_move_bookmark or tabz_delete_bookmark.`, SaveBookmarkSchema.shape, async (params) => {
        try {
...
Use this ID with \`tabz_move_bookmark\` or \`tabz_delete_bookmark\` if needed.`
    ...
    // Create folder
    server.tool("tabz_create_folder", `Create a new bookmark folder.
...
Use the returned folder ID with tabz_save_bookmark to add bookmarks to it.`, CreateFolderSchema.shape, async (params) => {
        try {
...
Use this ID as \`parentId\` in \`tabz_save_bookmark\` to add bookmarks to this folder.`
    ...
    // Move bookmark
    server.tool("tabz_move_bookmark", `Move a bookmark or folder to a different location.
...
Note: Cannot move the Bookmarks Bar or Other Bookmarks folders themselves.`, MoveBookmarkSchema.shape, async (params) => {
        try {
...
**New Location:** ${destName}${params.index !== undefined ? ` (position ${params.index})` : ''}`
    ...
    // Delete bookmark
    server.tool("tabz_delete_bookmark", `Delete a bookmark or folder.
...
This action cannot be undone through this tool.`, DeleteBookmarkSchema.shape, async (params) => {
        try {
