...
async function profilePerformance(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/debugger/performance`, { tabId: options.tabId }, { timeout: 30000 });
        return response.data;
    }
    catch (error) {
...
async function getCoverage(options) {
    try {
        const response = await axios.post(`${BACKEND_URL}/api/browser/debugger/coverage`, { tabId: options.tabId, type: options.type }, { timeout: 30000 });
        return response.data;
    }
    catch (error) {
 ...
 */
function formatCoverageFiles(files, type) {
    const lines = [];
    if (files.length === 0) {
