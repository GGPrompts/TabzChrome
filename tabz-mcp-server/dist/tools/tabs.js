...
import { BACKEND_URL, getCurrentTabId, setCurrentTabId, getCustomTabName, setCustomTabName } from "../shared.js";
...
export function registerTabTools(server) {
    // List tabs tool
    server.tool("tabz_list_tabs", `List all open browser tabs with ACCURATE active tab detection.
...
Error Handling:
  ...
  - Empty list: No web pages open (only chrome:// pages)`, ListTabsSchema.shape, async (params) => {
        try {
...
Only chrome:// or extension pages are present.`;
                    ...
                    const lines = [`# Browser Tabs (${result.tabs.length} open)`, ""];
    ...
    // Switch tab tool
    server.tool("tabz_switch_tab", `Switch to a specific browser tab.
...
Error Handling:
  ...
  - "Cannot connect": Ensure TabzChrome extension is installed and backend is running at localhost:8129`, SwitchTabSchema.shape, async (params) => {
        try {
...
Use \`tabz_list_tabs\` to see available tab IDs and which one is currently targeted.`;
    ...
    // Rename tab tool
    server.tool("tabz_rename_tab", `Assign a custom name to a browser tab.
...
Error Handling:
  - "Invalid tab ID": tabId doesn't exist (use tabz_list_tabs to see valid IDs)`, RenameTabSchema.shape, async (params) => {
        try {
