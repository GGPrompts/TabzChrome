 ...
 */
async function listTerminals(state) {
    try {
 ...
 */
async function findTerminal(nameOrId) {
 ...
 */
async function sendKeys(terminal, text, execute, delay) {
    ...
    try {
        ...
        const response = await axios.post(`${BACKEND_URL}/api/terminals/send-keys`, {
 ...
 */
async function captureTerminal(terminal, lines) {
    try {
        const response = await axios.get(`${BACKEND_URL}/api/terminals/${encodeURIComponent(terminal.id)}/capture`, {
            params: { lines },
            timeout: 5000
        });
        if (response.data.success) {
