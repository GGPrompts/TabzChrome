...
Only chrome:// or extension pages are present.`;
  ...
  server.tool(
    "tabz_switch_tab",
    `Switch to a browser tab by ID, making it Claude's target for screenshots/clicks/fills. Get IDs from tabz_list_tabs. Use tabz_docs for detailed docs.`,
...
Use \`tabz_list_tabs\` to see available tab IDs and which one is currently targeted.`;
  ...
  server.tool(
    "tabz_rename_tab",
    `Assign a custom name to a tab (stored by URL, survives tab reordering). Empty string clears. Session-scoped. Use tabz_docs for detailed docs.`,
